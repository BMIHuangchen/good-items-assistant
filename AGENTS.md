...
- 2026-06-13 线上服务器复查：生产数据库由 `/etc/good-items-api.env` 指向本机 `jdbc:mysql://127.0.0.1:3306/good_items_assistant`，实际为 MariaDB 10.11，数据目录为 `/var/lib/mysql/`，当前不是云数据库。
- 2026-06-13 线上数据库备份：已在服务器生成 `/root/good_items_assistant_backup_20260613_031301.sql`，后续任何线上数据库写入前仍应先重新备份。
...
- 所有线上数据库写入必须先确认目标库为 `good_items_assistant`，并先生成可回滚 SQL 备份。
